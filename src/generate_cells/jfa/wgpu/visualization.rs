...
static COLOR_MAP: Lazy<Mutex<HashMap<u32, Rgb<u8>>>> = Lazy::new(|| {
...
pub fn generate_image_visualization(buffer: &[u32], output_path: &str) -> Result<(), String> {
    ...
    let color_map = COLOR_MAP.lock().map_err(|e| format!("Failed to lock color map: {}", e))?;
...
fn ensure_color_map_has_all_ids(buffer: &[u32]) -> Result<(), String> {
    let mut map = COLOR_MAP.lock().map_err(|e| format!("Failed to lock color map: {}", e))?;
